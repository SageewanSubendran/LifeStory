...
class _StoryState extends State<Story> {
  ...
  checkMoodValue(){
    ...
    if(!widget.check){
      _currentItemSelected = "Pick a mood...";
    }
    else{
      _currentItemSelected = _data.documents['mood'];
    }
  }

  @override
  void didChangeDependencies() {
    // Provider.of<StoryProvider>(context).getStoryData(widget.date);
    checkMoodValue();
                ...
                child: Column(
                  children: [
                    textStory1FromField('Add a title..',
                      (_data.documents!=null)?_data.documents['title']:'',
                            (value){_title=value;},
                    ...
                    Row(
                      ...
                      children: [
                        dropDown(),
                        (widget.check)?GestureDetector(
                          onTap: ()async{
                            // final Email email = Email(
                            //                             //   body: 'title: ${_data.documents['title']} \n mood: ${_data.documents['mood']} \n story: ${_data.documents['story']}',
                            //                             //   subject: 'Story',
                            //                             //   isHTML: false,
                            //                             // );
                            //                             //
                    ...
                    Container(
                      ...
                      child: textStory2FromField('Click to write story...',
                        (_data.documents!=null)?_data.documents['story']:'',
                        (value){_story=value;},
                            (val)=> val.isEmpty ? 'enter story' : null,
                      ),
                    ),

                    SizedBox(height: (widget.check)?20.w:0.w,),

                    (widget.check)?GestureDetector(
                      onTap: (){
                        if(_data.documents['mediaPath']!=null){
                        if(_data.documents['mediaType']=="picture"){
                    ...
                    GestureDetector(
                      onTap: ()async{
                        ...
                        if (form.validate()) {
                          ...
                          if(!widget.check) {
                            if (_currentItemSelected != 'Pick a mood...') {
                              setState(() {
                                _showSpinner = true;
                              });
                              _data.addStory(context, StoryModel(
                                title: _title,
                            ...
                            _data.updateStory(context, StoryModel(
                              title: (_title!=null)?_title:_data.documents['title'],
                              mood: _currentItemSelected,
                              mediaPath: (_mediaPath!=null)?_mediaPath:_data.documents['mediaPath'],
                              mediaType: (_mediaType!=null)?_mediaType:_data.documents['mediaType'],
                              date: widget.date,
                              story: (_story!=null)?_story:_data.documents['story'],
                            ));
                          }
                        }
                      },
